...
import {Channel, EventEmitter} from "@ublitzjs/core/channel"
...
closure(() => {
  ...
  if( speedOf(myRes) / speedOf(tseepRes) < 1.7
    ...
    || speedOf(myRes) / speedOf(tseepSafeRes) < 1.08
...
closure(() => {
  ...
  if( speedOf(myRes) / speedOf(tseepRes) < 2.6
    ...
    || speedOf(myRes) / speedOf(tseepSafeRes) < 2
...
closure(() => {
  ...
  bench.add("channel", () => {
    ...
    tseepSafe.emit("foo", "SOME LARGE DATA")
  }).add("node:events", () => {
  ...
  if( speedOf(tseepRes) / speedOf(myRes) > 1.15
    || speedOf(tseepSafeRes) / speedOf(myRes) > 1.15
...
closure(() => {
  ...
  var tseep = new Tseep_ee();tseep.on("foo", bar).on("foo", baz).on("foo", foo)
  var tseepSafe = new TseepSafe_ee();tseepSafe.on("foo", bar).on("foo", baz).on("foo", foo)
  var node = new Node_ee(); node.on("foo", bar).on("foo", baz).on("foo",foo)
  var my = { 'foo': new Channel() }; my.foo.sub(bar); my.foo.sub(baz); my.foo.sub(foo)
  var cozy = new CozyEvent(); cozy.on("foo", bar); cozy.on("foo", baz); cozy.on("foo", foo)
  var bench = new Bench({ name: "add/remove 1 listener", time: 1000 });
  ...
  bench.add("channel", () => {
    ...
    tseepSafe.on("foo", dynamic)
    tseepSafe.emit("foo", payload)
    tseepSafe.emit("foo", payload)
    tseepSafe.emit("foo", payload)
    tseepSafe.emit("foo", payload)
    tseepSafe.emit("foo", payload)
    tseepSafe.off("foo", dynamic)
  }).add("node:events", () => {
    node.on("foo", dynamic)
  ...
  if(  speedOf(myRes)  / speedOf(tseepRes) < 6 // due to in-flight optimizations
    ...
    || (speedOf(myRes) / speedOf(tseepSafeRes)) < 1.2 
...
closure(()=>{
  ...
  bench.runSync()
  console.table(bench.table())
  var [tseepRes, tseepSafeRes, myRes, nodeRes, cozyRes] = bench.tasks
  if( speedOf(myRes) / speedOf(tseepRes) < 13
    || (speedOf(myRes) / speedOf(nodeRes)) < 3.8
    || (speedOf(myRes) / speedOf(tseepSafeRes)) < 3.5
