...
closure(() => {
  ...
  if (diff > 1.15) errors.push(new Error("NOT FAST CREATION"))
})
closure(() => {
...
closure(() => {
  ...
  console.table(bench.table())
  var [myRes, tseepRes, nodeRes, cozyRes] = bench.tasks
  if (speedOf(myRes) / speedOf(tseepRes) < 60
    || (speedOf(myRes) / speedOf(nodeRes)) < 60
    || (speedOf(myRes) / speedOf(cozyRes)) < 155
...
closure(() => {
  ...
  if (speedOf(myRes) / speedOf(tseepRes) < 5.5
    ...
    || (speedOf(myRes) / speedOf(cozyRes)) < 1.3
